...
기존 shell-script 기반 구현은 [`archive/shell/`](archive/shell/) 로 이관되어 reference 용으로만 보관된다. 새 구현은 `docs/` 의 contract 를 기준으로 다른 언어로 재작성될 예정이며, 그 전까지 active runtime 은 없다. docs 본문에 남아 있는 `application/...sh`, `lib/...sh`, `adapters/...sh` 등의 경로는 archive 트리를 가리키며, 새 구현이 자리 잡으면 매핑이 갱신된다.
