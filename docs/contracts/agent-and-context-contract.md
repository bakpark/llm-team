...
### Negative Space (다른 turn 의 작업 공간)

본 절은 inner forge `lead_draft` 외 turn 에 mutable workspace 를 부여하지 않음을 의미한다. middle review 의 reviewer 는 SliceMerge 기반 read-only checkout 을 받으며 `review_verdict` 만 산출한다 (patch 산출 없음 — `#AGC-OUTPUT` 매트릭스). request_changes 후 후속 patch 는 동일 Slice 의 새 inner build session 의 forge `lead_draft` 가 산출한다 (`state-and-operation-contract.md#SOC-SLICE-MERGE` Flow step 7 → step 1 재진입 — 새 SliceMerge 인스턴스).
