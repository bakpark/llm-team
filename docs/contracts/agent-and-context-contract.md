...
| 필드 | 필수 | 의미 |
|---|---|---|
| `output_kind` | yes | `spec_proposal`, `task_plan`, `patch`, `verdict`, `milestone_package`, `failure` 중 하나. enum 값 자체는 본 표가 권위 — 검증의 *위치* 는 `agent_role` 에 따른 허용 매핑(다음 행 참조)으로 위임된다 |
...
| `idempotency_key` | yes | Caller 가 enrichment 단계에서 합성하는 키. 합성 식은 `SOC-OPERATIONS` 가 operation 별로 정의한다 |
...
Caller 는 enrichment 단계에서 SOC-OPERATIONS 의 식에 따라 `idempotency_key` 를 합성하여 envelope 의 표준 위치에 기입한다. Agent 가 어떤 형태로든 이 필드를 산출했다면 Caller 는 자체 합성 결과로 덮어쓴다.
