...
| `SOC-OPERATIONS` | partial | `src/application/dialogue-coordinator.ts` (middle review `runOneMiddleReviewTurn`), `src/application/turn-worker.ts` (inner `runOneInnerTurn`), `src/application/caller-dispatch-outer.ts` (`dispatchOuterOutcome` for outer Discovery/Specification/Planning/Validation). outer dialogue-coordinator pickup + AWAITING_HUMAN binding은 phase 5b.2 | always_hard | 2026-05-05-loop (loop·purpose 기반 재구성) |
