...
| `SOC-OPERATIONS` | partial | `src/application/dialogue-coordinator.ts` (middle review `runOneMiddleReviewTurn`: pickup → manifest → invoke → persist → evaluate → dispatch), `src/application/turn-worker.ts` (inner `runOneInnerTurn`). outer Discovery / Specification / Planning / Validation 은 phase 5 | always_hard | 2026-05-05-loop (loop·purpose 기반 재구성) |
| `SOC-DISPATCH-MATRIX` | partial | `src/domain/dispatch-matrix.ts` (data-driven `DISPATCH_MATRIX` + `lookupDispatch`), `src/application/caller-dispatch.ts` (`dispatchOutcome` 실행자 — 인너 tests_green / TIMEOUT / ABANDONED + 미들 approve / request_changes 분기 커버). 외부 (outer) 분기는 phase 5 가 행 추가만으로 확장 | always_hard | 2026-05-05-loop ((state, final_verdict) tuple 분기) |
...
| `RGC-HUMAN-CONTRIBUTION` | partial | `src/application/human-signal-drain.ts` (envelope drain skeleton — `applied`/`invalid` markProcessed). human_approval contribution 변환 + 영속 큐 enqueue는 phase 5b dialogue-coordinator 가 담당 | always_hard (required human contribution) | 2026-05-05-loop (feature slice 한정 scope) |
...
| `KAC-DECISION-LOG` | partial | `src/domain/schema/knowledge.ts` (`DecisionEntry` + `DecisionKind` 6-kind enum), `src/application/persistence-layout.ts` (`layout.decision`). recordDecision use case + atlas/sentinel producer wiring은 phase 5b | always_hard | 2026-05-05-loop (decision_kind enum 확장) |
| `KAC-CONTEXT-SUMMARY` | partial | `src/domain/schema/knowledge.ts` (`ContextSummary` + `ContextSummarySliceRef`), `src/application/persistence-layout.ts` (`layout.contextSummary`). snapshotContextSummary on Validation pass는 phase 5c | always_hard | 2026-05-05-loop (slice telemetry 요약 포함) |
