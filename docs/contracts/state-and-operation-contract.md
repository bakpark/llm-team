...
- Caller action on approve: Code CP를 병합하고 `CP_MERGED`, Task를 `TASK_INTEGRATED`로 전이
- Caller action on request-changes: CP를 `CP_CLOSED`, Task를 `TASK_REJECTED -> TASK_READY`로 전이
