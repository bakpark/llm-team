...
- Caller action on lead submit: diff 수집, Code CP 생성, CP 를 `CP_READY_FOR_REVIEW` 로 전이, Task 를 `TASK_REVIEW_READY` 로 전이 (Implementation phase 의 quorum 은 lead_only — patch 자체가 다음 phase 의 입력)
