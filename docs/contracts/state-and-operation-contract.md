...
| 분기 | Ledger result |
|---|---|
| 위 분기표의 정상 종착(`success` 행으로 표기되는 모든 transition) | `success` |
...
같은 cycle 에서 두 result 가 동시에 후보가 되면 우선순위는 다음과 같다: `claim_failed` > `duplicate` > `noop` > `stale` > `invalid` > `error` > 정상 분기. 우선순위는 *최초로 만족한 조건* 으로 cycle 을 즉시 종료시키는 의미다.
