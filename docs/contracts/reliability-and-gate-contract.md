...
| `target.dual_track_priority` | `delivery_first` (default) / `balanced` / `discovery_first` — TCC-DUAL-TRACK |
...
cross-slot fairness 위반 (예: 한 slot 이 다른 slot 의 worker 를 starve) 은 ledger 에 별도 entry 로 기록되지 않으나, `loop_policies.<phase>.session_idle_seconds_p99` 같은 metric 으로 관측한다 (TCC-DUAL-TRACK).
