...
| `onboarding.self_hosting` | true면 target의 worktree가 Caller의 프레임워크 자체 코드를 가리키는 self-hosting 모드. 이 경우 추가 안전 장치(예: 회귀 검증 게이트)가 적용된다 |
...
self-hosting 모드는 `docs/contracts/reliability-and-gate-contract.md#RGC-FAILURE`의 partial-fail 정책 및 `#RGC-DAEMON-STARTUP`의 atomic 시작 정책과 결합한다. self-hosting 환경에서 기동된 worker가 자기 자신의 코드를 갱신하는 동안의 무결성은 본 contract가 단독으로 보증하지 않으며, 사람의 명시적 검토가 진입 게이트의 일부가 된다.
