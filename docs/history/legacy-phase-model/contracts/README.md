...
| **Quorum** | PhaseRun 의 최종화 조건. `phase_policies.<phase>.quorum` 으로 설정. enum: `lead_only`, `min_approvals`, `all_reviewers`, `any_request_changes_blocks`. enum 별 의미와 system default 는 `docs/contracts/target-config-contract.md#TCC-PHASE-POLICIES` 의 "Phase default" 절. |
