...
| `lead_draft` (inner tdd_build, code_tree mount) | `tree` | slice-local worktree read-write. 본문 snapshot/truncation 대상 아님 |
| `lead_draft` (rework — `parent_review_verdict_id` 보유) | `body+comments` | 직전 review_verdict 코멘트가 1급 입력 (rework_patch contribution_kind 는 폐기, lead_draft + parent_review_verdict_id 로 흡수됨) |
| `review_verdict` | `body+comments` | required_evidence (VerificationRun + MetricRun) + 직전 contribution 코멘트가 판단 입력 |
| `proposal` | `body+comments` | scout 의 evidence/refactor 제안 — required_evidence 의 producer (legacy `evidence` contribution_kind 흡수) |
| `human_approval` | `body` | 사람 검토 시점의 본문이 1차 입력 |

폐기된 contribution_kind: `rework_patch` (→ `lead_draft` + `parent_review_verdict_id`), `evidence` (→ `proposal` + RequiredEvidence 인프라), `summary` (→ outer Validation `lead_draft` artifact). 자세한 내용은 [`#AGC-CONTRIBUTION`](../contracts/agent-and-context-contract.md#AGC-CONTRIBUTION).

위 default 는 호출자가 명시적으로 더 좁힐 수 있다. 더 넓힐 때는 manifest entry 단위 명시가 필요하다. loop · phase / purpose 별 override 는 `loop_policies.<loop>.<phase|purpose>.fetch_scope_overrides` 가 우선한다.
...
- `fetch_scope=body+comments` 의 코스트가 큰 contribution_kind (`review_verdict`, rework `lead_draft` 등) 에서는 comments 페이지네이션 한도를 만지지 않는다. 누락된 댓글로 의사결정을 하면 reviewer drift 의 원인이 된다.
