...
> Status. 본 invariant 는 contract / prompt 형식 차원에서 정의되어 있다. 7 필드 동일성을 검증하는 application 측 envelope parser (runtime guard) 는 Stage 3a (Fake Runner MVP,  §14) 에서 구현 예정이다 — 본 PR 시점에는 contract-only 상태.
...
본 마이그레이션은  Stage 2 (Implementation Foundation) 에서 contract 변경과 함께 진행한다.
