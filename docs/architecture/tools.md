...
| Helper | 책임 |
|---|---|
| `lr_invoke` (`lib/ports/llm_runner.sh`) | Context Manifest와 AgentProfile prompt를 기반으로 1회 Agent 호출 (포트) |
| `agent_output_validate` (`lib/output.sh`) | AGC output envelope 1차 검증 |
| `agent_output_validate_extended` (`application/agent_io.sh`) | (parent_loop, contribution_kind) × output_kind 별 확장 검증 |
...
| Helper | 책임 |
|---|---|
| `it_issue_create` / `it_issue_set_state` | outer Planning session 산출 Slice 를 Issue 로 영속화, 상태 marker 갱신 |
| `change_proposal_create` (`lib/change_proposal.sh`) | Spec CP / Milestone CP artifact 영속화 (코드 병합 대상 아님). slice 의 코드 병합은 SliceMerge lifecycle 참조 |
| `change_proposal_set_state` | Spec CP / Milestone CP state marker 갱신(from_state 명시 시 전이 검증) |
| `it_milestone_set_state` | Milestone state marker 갱신 (dual-slot 의 Discovery/Delivery state) |
| `human_signal_drain` (`application/human_signal.sh`) | RGC-SIGNALS envelope 읽기·적용 |
| `nt_send` (`adapters/notifier/`) | push-only 알림 |

`gh pr review`, `gh pr merge`, `gh issue close`, label 변경은 모두 `it_*` 포트 어댑터를 통해서만 실행한다. trunk merge 는 `slice_merge.sh` 가 lifecycle state 를 따라 호출한다.

## Git Helpers

| Helper | 책임 |
|---|---|
| `workspace_create` | inner tdd_build 의 격리 worktree 생성 |
| `workspace_diff_collect` | inner tdd_build 의 forge turn workspace diff 수집 |
| `slice_merge_branch_create` | Slice 단위 병합 branch (SliceMerge) 생성 |
| `slice_merge_apply` | SliceMerge 를 trunk 에 deterministic merge/rebase 적용 |
| `merge_or_rebase_clean` | base 가 낡은 SliceMerge 의 deterministic merge/rebase 가능성 검사 |
| `workspace_remove` | workspace 정리 |

inner tdd_build 의 forge turn 은 workspace 내부 파일을 수정할 수 있지만, branch push 와 SliceMerge transition 은 Caller helper (caller_dispatch + slice_merge.sh) 가 수행한다.
...
middle review / SliceMerge revalidation / outer Validation 은 Verification Run log 를 해석한다. 검증 명령 실행은 Caller helper 가 수행한다 — inner tdd_build turn 직후 자동 트리거 + middle/outer 의 pre-action 트리거 두 경로.
...
| Helper | 호출 site |
|---|---|
| `context_manifest_create` / `context_manifest_add_entry` | turn worker cycle 단계 [3] (Manifest + Workspace + Prompt) |
| `context_manifest_validate` | turn worker LLM 호출 직전 |
| `lease_claim` / `lease_release` | dual_track_scheduler (slot_lock), dialogue_coordinator (session_lease), turn worker (turn_lease 또는 turn_index CAS) — `RGC-LEASE-KINDS` |
| `lease_expire_scan` | `application/recovery.sh` `recovery_scan()` |
| `verification_run_for` | `application/verification_runner.sh`, inner tdd_build turn 직후 + middle review / SliceMerge revalidation / outer Validation pre-action |
| `transition_ledger_write` | `application/caller_dispatch.sh::_caller_ledger_write`, `application/recovery.sh::_recovery_ledger_write`, turn worker `::_runner_ledger_write` |
