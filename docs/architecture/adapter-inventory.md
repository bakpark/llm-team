...
| `notifier` | `adapters/notifier/discord.sh` / `slack.sh` | Webhook + `curl` + `jq` (Discord embeds / Slack Block Kit) | `none.sh` (default no-op, **운영용**) | `LLM_TEAM_ADAPTER_NOTIFIER` 기본 `none`. caller 가 `load_target` 후 [`registry_rebind_for_target`](../../lib/registry.sh) 을 *명시 호출* 하면 `TARGET_NOTIFIER_CHANNEL` 값으로 rebind. 현재 알림 흐름에서는 `lib/notifier.sh` `notify_review_needed` 가 이를 수행한다. | production / default-no-op |
...
> 주: `target.yaml` 의 `adapters.*` 키 기반 매핑은 **현재 미구현** 이며 [`lib/registry.sh`](../../lib/registry.sh) `registry_rebind_for_target` 본문에 "추후" 로 표기되어 있다. 현재는 위 `LLM_TEAM_ADAPTER_*` 환경변수 + notifier 한정 `TARGET_NOTIFIER_CHANNEL` rebind 가 유일한 매핑 경로다.
...
- **Hot-reload 미지원**: 코드 갱신은 daemon 재기동 시 반영 ↦ [`self-hosting.md` §3 Hot-reload 미지원](self-hosting.md#3-hot-reload-미지원).
...
- **`target.yaml` `adapters.*` 매핑 미지원**: 현재 어댑터 선택은 `LLM_TEAM_ADAPTER_*` env + notifier 만 `TARGET_NOTIFIER_CHANNEL` rebind ↦ [`lib/registry.sh`](../../lib/registry.sh) `registry_rebind_for_target`.
...
- 신규 어댑터 = `adapters/<port>/<id>.sh` 한 파일. 본 파일은 [`lib/registry.sh`](../../lib/registry.sh) 의 `registry_load_adapter` 에 의해 *현재 셸로 source* 되어 함수를 정의한다 (별도 `export -f` 불필요). `registry_verify_port` 가 `lib/ports/<port>.sh` 의 `PORT_*_REQUIRED_FUNCTIONS` 각 함수 존재 여부를 `declare -F` 로 검증한다.
- 어댑터 식별자 (`<id>`) 활성화 경로 (현재 구현):
  ...
  - **llm_runner role-별 매핑**: contract 와 config helper 는 존재하지만 *runner 바인딩에는 미연결*. [`target-config-contract.md#TCC-AGENT-RUNNER-MAP`](../contracts/target-config-contract.md#TCC-AGENT-RUNNER-MAP) 가 `agent_runner.default` / `agent_runner.by_role` 을 정의하고 `lib/config.sh` `config_agent_runner_for_role` 가 lookup 헬퍼를 제공하나, `scheduler/runner.sh` 는 현재 이 헬퍼를 호출하지 않으며 `lib/common.sh` source 시점에 `LLM_TEAM_ADAPTER_LLM_RUNNER` 단일 값으로 바인딩된다. *active binding TBD*.
  - `target.yaml` 의 `adapters.*` 키 기반 매핑은 미구현 ([`lib/registry.sh`](../../lib/registry.sh) `registry_rebind_for_target` 본문 "추후" 표기).
...
- `target.yaml` `adapters.*` 매핑 ([`lib/registry.sh`](../../lib/registry.sh) `registry_rebind_for_target` "추후") 미구현 (ISSUE TBD).
- llm_runner role-별 매핑의 active binding ([`scheduler/runner.sh`](../../scheduler/runner.sh) 가 `config_agent_runner_for_role` 미호출) 미구현 (ISSUE TBD).
