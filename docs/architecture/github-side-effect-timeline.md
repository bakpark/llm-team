...
| inner tdd_build pre-action (Slice → SLICE_BUILDING entry) | 새 SliceMerge 인스턴스 `SM_DRAFT` 생성 + draft PR open (`external-tracking-mapping.md` §4). inner turn 별 workspace_commit push 는 [`worktree-pr-lifecycle.md`](worktree-pr-lifecycle.md) §4 | slice worktree 의 외부 push, label 충돌 |
