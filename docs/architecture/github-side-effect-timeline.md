...
| Phase quorum_reached 시 | 단계 [C] 의 GitHub write 순서 | TOCTOU 가능성 |
|---|---|---|
| Discovery | `it_milestone_create()` 또는 `it_milestone_update()` → `it_milestone_set_state()` | milestone description 의 동시 편집(거의 없음) |
| Specification | `it_milestone_update()` → `it_milestone_set_state()` | 위와 동일 |
| Planning | `it_issue_create()` × N → `it_issue_link_to_milestone()` × N → `it_issue_set_blocked_by()` × N → `it_milestone_set_state()` | issue 생성과 link 사이에 외부 사용자가 milestone 을 close 할 가능성 |
| Implementation (lead/rework_patch contribution submit) | (worktree 작업은 §3) → PR 생성/갱신 → `it_issue_set_state()` task → CP marker write | PR 본문/head SHA 의 외부 push, label 충돌 |
| CodeReview | CP marker write → label 갱신 → comment | PR head SHA 가 검증 시점 이후 push 로 변경됨(pin re-check 가 흡수) |
| Integration | milestone description 갱신 → `it_milestone_set_state()` → 필요 시 cleanup PR | integration 도중 외부 task 추가 |
| Validation | `it_milestone_set_state()` → close note 또는 release 발행 | 검증 도중 task 회귀 |
...
## 3. 워크스페이스 side-effect (Implementation phase)

Implementation phase 의 forge contribution 은 worktree 단계에서 다음을 수행한다.
...
worktree → push 사이에 [`docs/contracts/reliability-and-gate-contract.md#RGC-PHASE-LEASE`](../contracts/reliability-and-gate-contract.md#RGC-PHASE-LEASE) 의 contribution lease 가 만료되면 push 결과는 ledger 에 기록되지만 이후 단계는 stale 로 종결된다.
