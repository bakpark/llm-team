...
새 어댑터(예: `coder`, `qwen36`)는 `adapters/llm_runner/<id>.sh` 로 추가한다. 어댑터 식별자는 [`docs/contracts/target-config-contract.md#TCC-AGENT-RUNNER-MAP`](../contracts/target-config-contract.md#TCC-AGENT-RUNNER-MAP) 의 `agent_runner.default` / `agent_runner.by_role` 값과 일치해야 한다.
...
|---|---|
| `role` | role 식별자 문자열 |
| `operation` | operation 식별자 문자열 |
| `manifest_id` | `lib/context.sh` `context_manifest_id()` 가 반환한 식별자 |
| `prompt_ref` | `prompts/` 하위 경로 |
| `agent_cwd` | `lib/worktree.sh` 가 만든 격리 디렉토리 경로 |
| `timeout` | 초 단위 정수 |
| `idempotency_key` | Caller 가 lease_token 등으로 합성한 키 |
