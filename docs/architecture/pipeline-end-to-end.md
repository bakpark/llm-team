...
phase 종착 transition (CP 생성, 다음 phase 로의 milestone state 전이) 은 본 contribution cycle 이 아니라 phase coordinator cycle 이 dispatch 한다. 각 단계는 contract 의 책임을 하나씩 *적용* 만 한다. 단계 사이에서는 envelope · ledger · lease 가 인터페이스다.
...
contribution 을 `CONTRIB_IN_PROGRESS -> CONTRIB_SUBMITTED` 로 전이하고 envelope 을 영속 큐에 enqueue 한다. 본 단계는 phase 종착 transition 을 일으키지 않으며, 그 책임은 phase coordinator cycle 이 진다. lead contribution 이 첫 submit 되면 milestone 의 phase_state 도 `*_DRAFT` 또는 `*_AWAITING_QUORUM` 으로 진입할 수 있는데, 그 전이는 phase coordinator 의 책임이다.
...
- `quorum_reached` → lead contribution 의 산출을 final artifact 로 압축하고 `SOC-DISPATCH-MATRIX` 의 phase 종착 row 를 dispatch (CP 전이, milestone state 전이, 자식 객체 처리). `quorum_decision=quorum_reached` 로 ledger 기록.
